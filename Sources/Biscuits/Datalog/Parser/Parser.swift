...
struct Parser {
    ...
    mutating func parseQuery(_ f: (String) -> ()) throws -> Biscuit.Query {
    ...
    mutating func parseRule(_ head: Predicate, _ f: (String) -> ()) throws -> Rule {
        var predicates: [Predicate] = []
        var expressions: [Biscuits.Expression] = []
        var origins: [TrustedScope] = []
        try self.parsePredicateOrExpression(
    ...
    mutating func parsePredicateOrExpression(
        ...
        switch try self.at() {
            case .keyword(let kw):
                self.bump()
                let potentialValue: Value? = switch kw {
    ...
    mutating func parsePredicate(_ name: Substring, _ f: (String) -> ()) throws -> Predicate {
    ...
    mutating func parseExpression(_ f: (String) -> (), _ precedence: Int = 0, _ lhs: Expression? = nil) throws -> Expression {
    ...
    mutating func parseExpressionPrefix(_ f: (String) -> (), _ lhs: Expression? = nil) throws -> Expression {
        if let lhs = lhs {
    ...
    mutating func parseExpressionMethod(_ f: (String) -> (), _ lhs: Expression? = nil) throws -> Expression {
        ...
        enum MethodType { case unary(OpUnary), binary(OpBinary), closure(OpBinary), tryOr }

        var expr: Expression = try lhs ?? .term(self.parseTerm(f))
        while case .ptPeriod = try self.at() {
            self.bump()
            let name: Substring = switch try self.eat() {
                case .name(let n): n
            ...
            let methodType: MethodType = switch name {
    ...
    mutating func parseClosure(_ f: (String) -> ()) throws -> Closure {
    ...
    mutating func parseTerm(_ f: (String) -> ()) throws -> Term {
        switch try self.eat() {
